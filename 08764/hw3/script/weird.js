...
weird = (function() {
	weird = window.weird || {};
	/*
	 * get element by id @param string @return HTMLElement
	 */
	 ...
	 *            element optional start element, default document
	 * @param {string}
	 *            tagName optional
	 * @return {Array}
	 */
	weird.qEl = function(className, element, tagName) {
		var result = [], trim = weird.trim, len, i, elements, node;
		...
		if (element.getElementsByClassName) {
			...
			className = new RegExp("(^|\\s)" + weird.escapeReg(className)
					+ "(\\s|\x24)");
	 ...
	 * 
	 * @param string|HTMLElement @return Array
	 */
	 ...
	 * 
	 * @param string|HTMLElement @return HTMLElement
	 */
	...
	/*
	 * @param string|HTMLElement string @return bool
	 */
	weird.hasClass = function(el, className) {
		el = weird.gEl(el);
		var classArray = weird.trim(className).split(/\s+/), len = classArray.length;
	...
	/*
	 * @param string|HTMLElement @param string className @return HTMLElement
	 */
	...
	/*
	 * @param string|HTMLElement @param string @return HTMLElement
	 */
	weird.removeClass = function(el, className) {
		el = weird.gEl(el);
		var classArray = weird.trim(el.className).split(/\s+/), len = classArray.length, clazz = weird
				.trim(className), i, item;
	 ...
	 * foreach
	 * 
	 * @param {Array} array @param {Function} fun @return {Array}
	 */
	...
	function onSignUp(form, ev, prop) {
		...
		if (form.email && weird.trim(form.email.value) == "") {
			...
			for ( var i in form.elements) {
				...
				if (text && text.type && text.type == "text"
						&& text.name != "password") {
					if (!(text.value.indexOf('<') < 0)
							| !(text.value.indexOf('>') < 0)) {
						text.value = "";
						alert("Input can not contain \< or \>");
						return false;
					}
				}
			}
	...
	weird.each(joinuss, function(joinus) {
		joinus.onclick = function(ev) {
			goPage(ev, "register.jsp");
		};
	});
	// go to index.htm
	...
	weird.each(gsearchs, function(gsearch) {
		gsearch.onclick = function(ev) {
			goPage(ev, "search.jsp");
		};
	});
	...
	if (signin_form)
		signin_form.onsubmit = function(ev) {
			return onSignUp(this, ev, "Log In");
		};
	...
	weird.each(signups, function(signup) {
		...
		if (signup_form.name == "edit") {
			signup_form.onsubmit = function(ev) {
				return onSignUp(this, ev, "Edit");
			};
		} else if (signup_form.name == "joinus") {
			signup_form.onsubmit = function(ev) {
				return onSignUp(this, ev, "Sign Up");
			};
		} else if (signup_form.name == "story") {
			signup_form.onsubmit = function(ev) {
				return onSignUp(this, ev, "Post Weird");
			};
		} else if (signup_form.name == "comment") {
			signup_form.onsubmit = function(ev) {
				return onSignUp(this, ev, "Post Comment");
			};
		}
